...
{
	echo '<h1 class="access">'.__('You do not have access to this page, please make login or change your login account', 'flacso').'</h1>';
}
else 
{
	...
	{
		$post = $document->get_default_post_to_edit( $post_type, $publish );
	}
	...
	if($publish)
	{
		...
		foreach ($taxonomies as $taxonomy)
		{
			if(array_key_exists('taxonomy_'.$taxonomy, $_POST))
			{
				...
				{
					$message[] = __('error on set post taxonomy', 'flacso').': '.$taxonomy;
					$notice = true;
				}
			}
		}
		...
		if ($_FILES) {
			foreach ($_FILES as $file => $array) {
				if ($_FILES[$file]['error'] !== UPLOAD_ERR_OK && $notice)
				{
					...
					{
						case 'thumbnail':
							$message[] = __('error on set post thumbnail image', 'flacso');
							$has_thumbnail = false;
							break;
						case 'thumbnail2':
							$message[] = __('error on set post header image', 'flacso');
							$has_thumbnail2 = false;
							break;
						default:
							$message[] = __('error on set post images', 'flacso');
							break;
					}
		...
		foreach ($attach_id as $key => $value)
		{
			...
			if($key == 'thumbnail' && $has_thumbnail)
			{
				...
				{
					$message[] = __('error on set post thumbnail', 'flacso');
					$notice = true;
				}
			}
		...
		{?>
			<div class="new-document-sucess"><?php _e('A new document was successfully created and waiting approval, thanks'); ?>
			</div><?php
			return ;
		}
	}
		...
		switch ($type)
		{
			...
			default:	
				...
				<div class="document-item">
					<label for="<?php echo $id ?>" class="document-item-label">
							<div class="document-item-title"><?php echo $label;
								if(array_key_exists( 'required', $field ) && $field['required'])
								{?>
									<span class="document-item-required-asterisk">*</span><?php
								}?>
							</div>
						<div class="document-item-tip-text"><?php echo $tip; ?>
						</div>
					</label>
					<input type="text" id="<?php echo $id ?>" class="document-item-input-text <?php echo $type == 'date' ? 'hasdatepicker' : ''; ?> <?php echo $input_class ?>" value="<?php echo array_key_exists($id, $_REQUEST) ? wp_strip_all_tags($_REQUEST[$id]) : ''; ?>" name="<?php echo $id ?>">
					<div class="document-item-error-message"></div>
					<div class="document-item-required-message"><?php echo $required_message; ?></div>
				</div>
				<?php
			break;
			case 'wp_editor':
				...
				<div class="document-item">
					<label for="<?php echo $id ?>" class="document-item-label">
						<div class="document-item-title"><?php echo $label; ?>
							<span class="document-item-required-asterisk">*</span>
						</div>
						<div class="document-item-tip-text">
							<?php echo $tip; ?>
						</div>
					</label>
					<?php wp_editor((array_key_exists($id, $_POST) ? stripslashes($purifier->purify($_POST[$id])) : ''), $id,  array( 
				       'tinymce' => array( 
				            'content_css' => get_stylesheet_directory_uri() . '/inc/documents/css/editor-styles.css' 
				    		)
						)
					); ?>
					<div class="document-item-error-message"></div>
					<div class="document-item-required-message">
	...
	<div class="images"> <?php //TODO Make image a field ?>
		<div class="images-thumbnail">  
			<label for="thumbnail" class="document-item-label">
				<div class="document-item-title"><?php _e('Highlight Image', 'flacso'); ?>
				</div>
				<div class="document-item-tip-text">
					<?php _e('Image showed on listing, like on home or catalog!', 'flacso'); ?>
				</div>
			</label>
		...
		<div class="images-thumbnail2">
			<label for="thumbnail2" class="document-item-label">
				<div class="document-item-title"><?php _e('Header Image', 'flacso'); ?>
				</div>
				<div class="document-item-tip-text">
					<?php _e('Image showed on Header!', 'flacso'); ?>
				</div>
			</label>
	...
	<div class="category-group">
		<?php 
		Documents::taxonomy_checklist();
		?>
	</div>
	
	<input id="original_publish" type="hidden" value="Publish" name="original_publish">
	<input id="publish" class="button button-primary button-large" type="submit" accesskey="p" value="<?php _e('Publish Document', 'flacso'); ?>" name="publish">
	</form>
	</div>
<?php 
}
